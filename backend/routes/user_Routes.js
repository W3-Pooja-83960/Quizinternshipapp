...
const { STAFF_TABLE, STUDENTS_TABLE } = require("../config");
...
router.post("/student-register", (req, resp) => {
  ...
  // STEP 1: check is user already exists
  const checkSQL = `SELECT * FROM ${STUDENTS_TABLE} WHERE email = ?`;
  ...
  pool.query(checkSQL, [email], (error, result) => {
    ...
    // STEP 2: insert new user into database
    const insertSQL = `INSERT INTO ${STUDENTS_TABLE} (firstName, lastName, email, password) VALUES (?, ?, ?, ?)`;

    pool.query( insertSQL, [firstName, lastName, email, hashPassword], (error, insertResult) => {
        if (error) return resp.send(errorResponse(error));
...
router.post("/student-login", (req, res) => {
  ...
  // STEP 1: fetch user by email
  const sql = `SELECT * FROM ${STUDENTS_TABLE} WHERE email = ?`;
